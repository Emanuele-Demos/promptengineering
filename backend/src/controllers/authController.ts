...
import { removeMemberAvatar, setMemberAvatar } from '../services/avatarService'
...
export async function uploadAvatarHandler(
  ...
  try {
    ...
    if (!file) {
      res.status(400).json({ message: 'Nessuna immagine caricata' })
      return
    }

    await setMemberAvatar(userId, file.filename)
    const user = await getAuthUserById(userId)
    ...
    res.json({ user, message: 'Foto profilo aggiornata' })
  } catch (error) {
    next(error)
  }
}
...
export async function deleteAvatarHandler(
  ...
  try {
    const userId = res.locals.userId as number
    await removeMemberAvatar(userId)
    const user = await getAuthUserById(userId)
    ...
    res.json({ user, message: 'Foto profilo rimossa' })
  } catch (error) {
    next(error)
  }
}
