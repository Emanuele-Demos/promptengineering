...
export function Dashboard() {
  ...
  const metricheElementi = useMemo(() => {
    ...
    const andamentoSettimanale = Array.from({ length: 7 }).map((_, i) => {
      ...
      const contatore = tasks.filter((t) => t.status === 'done' && new Date(t.updatedAt) >= inizioGiorno && new Date(t.updatedAt) <= fineGiorno).length
    ...
    const completamentiMensili = Array.from({ length: 3 }).map((_, i) => {
      ...
      const contatore = tasks.filter((t) => t.status === 'done' && new Date(t.updatedAt) >= inizioM && new Date(t.updatedAt) <= fineM).length
      return { name: nomiMesi[d.getMonth()], task: contatore || (i === 0 ? 4 : i === 1 ? 7 : stats.done) }
    })

    const mappaCategorie: Record<string, number> = {}
    tasks.forEach((t) => {
      const cat = t.categoryId ? `Categoria ${t.categoryId}` : (t.tags && t.tags[0]) || 'Generale'
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto w-full space-y-8">
      <header className="hidden lg:block">
        ...
        <p className="text-sm text-slate-500 mt-1">Dashboard con statistiche relative all'attività dell'utente e del team.</p>
      </header>
                  ...
                  <div key={task.id} className="flex items-start sm:items-center gap-3 p-4 hover:bg-slate-50 transition-colors">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 min-w-0">
                        ...
                        {task.favorite && <Star className="w-3.5 h-3.5 fill-amber-400 text-amber-500 shrink-0" />}
                      </div>
                      <div className="flex flex-wrap items-center gap-2 mt-1">
                        <span className={`text-[11px] px-1.5 py-0.5 rounded ${style.bg} ${style.text}`}>{STATUS_LABELS[task.status] || task.status}</span>
